...
for btn_text, row, col, *action in buttons:
    cmd = (calculate if action and action[0] == "eval" else
           ...
           lambda v=btn_text: button_click(v))
    button = ttk.Button(root, text=btn_text, command=lambda bt=cmd, btn=None: [bt(), play_sound()])
    button.config(command=lambda bt=cmd, btn=button: [bt(), play_sound(), animate_button_press(btn)])
